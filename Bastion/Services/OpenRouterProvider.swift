...
//  Bastion
//
//  Copied VERBATIM from AIStudio (kochj23/AIStudio) with only the app-attribution
//  constants adapted (referer / title / Keychain service). Shared across Jordan
//  Koch's apps so the frontier-model path behaves identically everywhere.
//
...
enum OpenRouterProvider {
    ...
    static let referer = "https://github.com/kochj23/Bastion"
    static let title = "Bastion"

    /// macOS Keychain service used to store the OpenRouter API key.
    static let keychainService = "com.jkoch.bastion.openrouter"
