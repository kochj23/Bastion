...
//  and the Keychain store round-trip. Adapted from AIStudio.
//
...
final class OpenRouterFailoverTests: XCTestCase {
    ...
    func testOpenRouterAuthHeaders() {
        ...
        XCTAssertEqual(headers["HTTP-Referer"], "https://github.com/kochj23/Bastion")
        XCTAssertEqual(headers["X-Title"], "Bastion")
    }
    ...
    func testOpenRouterRequestConstruction() throws {
        ...
        XCTAssertEqual(request.value(forHTTPHeaderField: "HTTP-Referer"), "https://github.com/kochj23/Bastion")
        XCTAssertEqual(request.value(forHTTPHeaderField: "X-Title"), "Bastion")
    ...
    func testKeychainRoundTrip() {
        let store = KeychainStore(service: "com.jkoch.bastion.tests.\(UUID().uuidString)", account: "apiKey")
        store.delete()
